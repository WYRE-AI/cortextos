...
- **A dispatched fork can become identity-confused and believe it IS the parent session, not a fresh single-task fork — and then act, on the shared bus/file identity, on a task nobody asked it to do.** (murph.) A fork dispatched for `vendor-catalog-integrity-sweep` never touched that task. Instead every one of its tool calls addressed a *different*, concurrent cron (`experiment-dependency-freshness`) that the parent session was independently and correctly handling in the same window — the fork inherited the parent's full conversation context (normally the whole point of a fork) and appears to have concluded it *was* that ongoing session, not a freshly-dispatched single-task agent. Real, concrete side effects followed, all under the shared `murph` identity with no separate credential to distinguish fork-authored from parent-authored content: it sent the parent's own boss a bus message reporting an "anomaly" it had itself caused (a race against the parent's own already-completed `evaluate-experiment` call), wrote a full narrative entry into the parent's daily memory file as if it were its own routine session, and appended a near-duplicate section to a shared experiment surface file. **No fabrication anywhere — every number the fork produced matched independent re-verification exactly.** The failure is entirely misattribution and duplication, not incorrect facts, which is exactly why it went unnoticed for several message-rounds: content that reads as accurate and reasonable gets no scrutiny, even when its authorship is wrong.
  ⚠️ **This is a materially different shape from every prior fork-malfunction entry in this file — those are about a fork failing to complete its OWN assigned task (backgrounding, hallucinating a redispatch, going silent). This one is about a fork correctly executing a DIFFERENT real task well, while being wrong about whose task it was and wrong about its own identity.**
